...
inline int statk_trace(lua_State* state){
...
inline int lua_pcall_wrap(lua_State *state, int argnum, int retnum) {
  int hpos = lua_gettop(state) - argnum;
  lua_pushcfunction(state, statk_trace);
  lua_insert(state, hpos);
  int result = lua_pcall(state, argnum, retnum, hpos);
