...
namespace utils {
    ...
    public:
        ...
        public:
            ...
            explicit reverse_iterator(node* n)
                : node_(n)
            {
            }
            ...
            {
                node_ = node_->levels_[0].prev_;
                if (node_->levels_[0].prev_ == nullptr) {
                    node_ = nullptr;
                }
        ...
        std::pair<iterator, iterator> find(const Score& min, const Score& max, size_t max_item_size = 100)
        {
            ...
            if (max >= min) {
                ...
                for (int i = current_height_ - 1; i >= 0; i--) {
                    while (dummy->levels_[i].next_ && ((is_asc_ == -1 && dummy->levels_[i].next_->kv_.second > max) || (is_asc_ == 1 && dummy->levels_[i].next_->kv_.second < min))) {
                        dummy = dummy->levels_[i].next_;
                    }
                }
                size_t total = 0;
                dummy = dummy->levels_[0].next_;
                while (dummy && ((is_asc_ == -1 && dummy->kv_.second >= min) || (is_asc_ == 1 && dummy->kv_.second <= max))) {
                    if (total == 0) {
    ...
    private:
        iterator erase_(iterator it)
        {
            ...
            if (n == tail_) {
                tail_ = n->levels_[0].prev_ == head_ ? nullptr : n->levels_[0].prev_;
            }
        ...
        iterator find_(const value_type& k, node* bpos = nullptr, node* epos = nullptr, std::vector<node*, Alloc<node*>>* cache = nullptr) const
        {
            ...
            for (int i = h - 1; i >= 0; i--) {
                for (;;) {
                    if (epos == nullptr && dummy->levels_[i].next_ == nullptr) {
                        break;
                    } else if (epos != nullptr && (dummy->levels_[i].next_ == epos || dummy->levels_[i].next_ == nullptr)) {
                        break;
                    }
        ...
        iterator insert_(const value_type& kv, std::vector<node*, Alloc<node*>>& cache)
        {
            ...
            if (newptr->levels_[0].next_ == nullptr) {
                tail_ = newptr;
            }
