/**
	 1. 支持所有stl类型容器，接口简单只有一个函数，业务方传入容器变量即可使用
	   ...
	   std::string str = shynet::utils::Stringify_stl(vec);
	   // "[1,2,3]"

	   std::map<int, int> m;
	   m[1] = 100; m[2] = 200; m[3] = 300;
	   str = shynet::utils::Stringify_stl(m);
	   // "{1:100,2:200,3:300}"
	   ```
	...
	2. 支持多维嵌套容器
	  ...
	  str = shynet::utils::Stringify_stl(vec);
	  // "[[1,2,3],[4,5,6]]"
	  ```
	...
	3. 支持容器元素为自定义类型
	 ...
	 4. 支持自定义样式
	   ...
	   ```
	   // 自定义样式举例
	   // shynet::utils::STRINGIFY_STL_STYLE_ONE_BEAUTY 是我提供的一个默认带缩进换行的样式，业务方可以自己构造一个
	   // Stringify_stl_one_style 类型的变量实现更高定制化的样式
	   std::string str = shynet::utils::Stringify_stl(vec, shynet::utils::STRINGIFY_STL_STYLE_ONE_BEAUTY);
	   // [
	   //   "1",
	   //   "2",
	   //   "3"
...
#define STRINGIFY_STL_NONITERATIVE(val, style, posfunc) \
  if (val.empty()) { return std::string(); } \
  std::ostringstream ss; \
  ss << style.delimiter_whole_prefix_; \
  for (; ; ) { \
  ...
  return ss.str();
		...
		std::string Stringify_stl(std::queue<T> val,
			...
			STRINGIFY_STL_NONITERATIVE(val, style, front);
		}
