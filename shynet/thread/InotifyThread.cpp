...
namespace shynet {
	...
	namespace thread {
				...
				if (ev.len > 0 && iobuf_->inputbuffer()->length() >= ev.len) {
					...
					if (iter != task_map_.end()) {
						...
						if (fdpaths_iter != fdpaths.end()) {
							...
							if (ev.mask & IN_ACCESS) LOG_TRACE << "文件被访问 " << path;
							if (ev.mask & IN_ATTRIB) LOG_TRACE << "文件属性发生变化 " << path;
							if (ev.mask & IN_CLOSE_NOWRITE) LOG_TRACE << "以非可写的方式打开后关闭了文件 " << path;
							if (ev.mask & IN_CLOSE_WRITE) LOG_TRACE << "以可写的方式打开后关闭了文件 " << path;
							if (ev.mask & IN_CREATE) LOG_TRACE << "监控目录下新建了子文件、子目录 " << path;
							if (ev.mask & IN_DELETE) LOG_TRACE << "监控目录下删除了子文件、子目录 " << path;
							if (ev.mask & IN_DELETE_SELF) LOG_TRACE << "监控目录被删除 " << path;
							if (ev.mask & IN_IGNORED) LOG_TRACE << "文件被忽略  " << path;
							if (ev.mask & IN_ISDIR) LOG_TRACE << "针对目录发生的事件 " << path;
							if (ev.mask & IN_MODIFY) LOG_TRACE << "文件被修改 " << path;
							if (ev.mask & IN_MOVED_FROM) LOG_TRACE << "文件移出监控目录 " << path;
							if (ev.mask & IN_MOVED_TO) LOG_TRACE << "文件移入监控目录 " << path;
							if (ev.mask & IN_OPEN) LOG_TRACE << "文件被打开 " << path;
		...
		void InotifyThread::add(std::shared_ptr<task::NotifyTask> task) {
			...
			g_task = task.get();
			std::lock_guard<std::mutex> lock(task_mutex_);
			if (nftw(task->path().c_str(), [](const char* pathname, const struct stat* sbuf, int type, struct FTW* ftwb) -> int {
				if (type == FTW_D) {
