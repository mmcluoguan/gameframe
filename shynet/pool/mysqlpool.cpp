...
namespace shynet {
namespace pool {
    MysqlPool::MysqlPool(const mysqlx::SessionSettings& opt, size_t capactiy, size_t max_capactiy)
        : option_(opt)
    {
        capactiy_ = capactiy;
        max_capactiy_ = max_capactiy;
        fun_ = std::bind(&MysqlPool::reclaim, this, std::placeholders::_1);
        for (size_t i = 0; i < capactiy_; i++) {
