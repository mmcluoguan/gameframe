...
void ConsoleHandler::getgamelist(const OrderItem& order, int argc, char** argv)
{
    ...
    if (world != nullptr) {
        world->send_json(frmpub::FilterData::JsonValue { frmpub::JosnMsgId::GETGAMELIST_ADMIN_WORLD_C },
            { frmpub::JosnMsgId::GETGAMELIST_ADMIN_WORLD_S,
                [&](auto data, auto enves) -> int {
                    return world->getgamelist_admin_world_s(data, enves);
                } });
...
void ConsoleHandler::noticeserver(const OrderItem& order, int argc, char** argv)
{
    ...
    if (world != nullptr) {
        ...
        world->send_json(frmpub::FilterData::JsonValue { frmpub::JosnMsgId::NOTICESERVER_ADMIN_WORLD_C },
            { frmpub::JosnMsgId::NOTICESERVER_ADMIN_WORLD_S,
                [&](auto data, auto enves) -> int {
                    return world->noticeserver_admin_world_s(data, enves);
                } });
...
void ConsoleHandler::sysemail(const OrderItem& order, int argc, char** argv)
{
    ...
    if (world != nullptr) {
        ...
        if (msgc.IsObject()) {
            world->send_json(frmpub::FilterData::JsonValue { frmpub::JosnMsgId::SYSEMAIL_ADMIN_WORLD_C },
                { frmpub::JosnMsgId::SYSEMAIL_ADMIN_WORLD_S,
                    [&](auto data, auto enves) -> int {
                        return world->sysemail_admin_world_s(data, enves);
                    } });
