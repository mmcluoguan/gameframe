			...
			try {
				...
				if (old_gate_sid != "0") {
					if (old_gate_sid != gate_sid) {
						//跨服顶号处理
						auto gate = shynet::Singleton<DbClientMgr>::instance().find(old_gate_sid);
						if (gate != nullptr) {
							protocc::repeatlogin_client_gate_s notify;
							notify.set_aid(accountid);
							gate->send_proto(protocc::REPEATLOGIN_CLIENT_GATE_S, &notify);
							LOG_DEBUG << "发送消息" << frmpub::Basic::msgname(protocc::REPEATLOGIN_CLIENT_GATE_S) << "到"
								<< frmpub::Basic::connectname(gate->sif().st());
						}
					}
				}
				if (result == 0) {
